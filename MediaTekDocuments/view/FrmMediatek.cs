...

{
    ...
    public partial class FrmMediatek : Form
    {
        ...
        const string LIVREE = "livrée";
        const string REGLEE= "réglée";
        ...
        {
            lesLivres = controller.GetAllLivres();
            txbLivreCommandeNumero.Text = "";
            AccesReceptionLivreGroupBox(false);
            grpModifierSuiviCommande.Visible = false;
        }
        ...
        {
            string idCmdLivre = txbLivreCommandeNumero.Text;
            lesCommandes = controller.GetCommandes(idCmdLivre);
            RemplirCmdLivresListe(lesCommandes);
        ...
        /// </summary>
        /// <param name="acces">true ou false</param>
        private void AccesReceptionLivreGroupBox(bool acces)
        {
            grpCommandeLivre.Enabled = acces;
            txbCommandeNumero.Text = "";
            txbCommandeLivreMontant.Text = "";
            txbCommandeLivreNbExemplaires.Text = "";
            dtpReceptionExemplaireDate.Value = DateTime.Now;
        }
        ...
        private void AfficheReceptionLivresInfos(Livre livre)
        {
            // informations sur le livre
            txbLivreCommandeTitre.Text = livre.Titre;
            txbLivreCommandeAuteur.Text = livre.Auteur;
            txbLivreCommandeISBN.Text = livre.Isbn;
            txbLivreCommandeCollection.Text = livre.Collection;
            txbLivreCommandeGenre.Text = livre.Genre;
            txbLivreCommandePublic.Text = livre.Public;
            txbLivreCommandeRayon.Text = livre.Rayon;
            txbLivreCommandeImage.Text = livre.Image;

            string image = livre.Image;
            try
            {
                pcbLivreCommandeImage.Image = Image.FromFile(image);
            }
            catch
            {
                pcbLivreCommandeImage.Image = null;
            }
        ...
        {
            if (!txbLivreCommandeNumero.Text.Equals(""))
            {
        ...
        private void RemplirCmdLivresListe(List<CommandeDocument> commandes)
        {
            if (commandes != null)
            {
                ...
                dgvLivreCommandesListe.DataSource = bdgCommandesListe;
        ...
        private void txbLivreCommandeNumero_TextChanged(object sender, EventArgs e)
        {
            ...
            txbLivreCommandeAuteur.Text = "";
            txbLivreCommandeISBN.Text = "";
            txbLivreCommandeCollection.Text = "";
            txbLivreCommandeGenre.Text = "";
            txbLivreCommandePublic.Text = "";
            txbLivreCommandeRayon.Text = "";
            txbLivreCommandeImage.Text = "";
            pcbLivreCommandeImage.Image = null;
            RemplirCmdLivresListe(null);
            AccesReceptionLivreGroupBox(false);
            grpModifierSuiviCommande.Visible = false;
        }
        /// <summary>
        ...
        {
            if (!txbCommandeNumero.Text.Equals("") || !txbLivreCommandeNumero.Text.Equals(""))
            {
                try
                {
                    string id = txbCommandeNumero.Text;
                    DateTime dateCommande = dtpCommandeLivreDate.Value;
                    string montant = txbCommandeLivreMontant.Text;
                    int nbExemplaire = int.Parse(txbCommandeLivreNbExemplaires.Text);
                    string idLivre = txbLivreCommandeNumero.Text;
                    CommandeDocument commandedocument = new CommandeDocument(id, dateCommande, montant, nbExemplaire, idLivre , "00001", ENCOURS);
                ...
                {
                    MessageBox.Show("le nombre d'exemplaire doit être numérique", "Information");
                    txbCommandeLivreNbExemplaires.Text = "";
                    txbCommandeLivreNbExemplaires.Focus();
                }
            }
            else
            {
                MessageBox.Show("numéro du livre commandé obligatoire", "Information");
            }
        }
        ...
        private void dgvLivreCommandesListe_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                grpModifierSuiviCommande.Visible = true;
                AfficherSuiviCommande(commandeDocument);
            }
            catch
            {
                VideLstSuiviCommande();
            }
        }
        ...
        private void AfficherSuiviCommande(CommandeDocument commandeDocument)
        {
            ...
            {
                VideLstSuiviCommande();
                lstSuiviCommande.Items.Add(LIVREE);
                lstSuiviCommande.Items.Add(RELANCEE);
            }
            else if (commandeDocument.IdSuivi == "00002")
            {
                VideLstSuiviCommande();
                lstSuiviCommande.Items.Add(REGLEE);
            }
            else if (commandeDocument.IdSuivi == "00003")
            {
                VideLstSuiviCommande();
                lstSuiviCommande.Items.Add("Votre commande est réglée");
            }
            else if (commandeDocument.IdSuivi == "00004")
            {
                VideLstSuiviCommande();
        ...
        { 
            ...
            {
                commandedocument.IdSuivi = "00001";
                commandedocument.Suivi = ENCOURS;
            }
            else if (lstSuiviCommande.SelectedItem.ToString() == LIVREE)
            {
                commandedocument.IdSuivi = "00002";
                commandedocument.Suivi = LIVREE;
            }
            else if (lstSuiviCommande.SelectedItem.ToString() == REGLEE)
            {
                commandedocument.IdSuivi = "00003";
                commandedocument.Suivi = REGLEE;
            }
            else if (lstSuiviCommande.SelectedItem.ToString() == RELANCEE)
            {
                commandedocument.IdSuivi = "00004";
                commandedocument.Suivi = RELANCEE;
            }
            controller.SetCommandeDocument(commandedocument);
        ...
        {
            CommandeDocument commandeDocument = (CommandeDocument)bdgCommandesListe.List[bdgCommandesListe.Position];
            ModifierSuiviCommande(commandeDocument);
            AfficherSuiviCommande(commandeDocument);
            AfficheReceptionCommandesLivre();
        }
        ...
        {
            CommandeDocument commandeDocument = (CommandeDocument)bdgCommandesListe.List[bdgCommandesListe.Position];
            if (!commandeDocument.Suivi.Equals(LIVREE))
            {
                ...
                {
                    controller.SuppComandeDocument(commandeDocument);
                    AfficheReceptionCommandesLivre();
                }
            }
            else
        ...
        private void CommandeLivreeExemplaire(CommandeDocument commandedocument)
        {
            ...
            {
                for (int i = 0; i < commandedocument.NbExemplaire; i++)
                {
                    int numero = int.Parse(commandedocument.IdLivreDvd) + 1000 + i;
                    DateTime dateAchat = commandedocument.DateCommande;
                    string idEtat = ETATNEUF;
                    string idDocument = commandedocument.IdLivreDvd;
                    Exemplaire exemplaire = new Exemplaire(numero, dateAchat, "", idEtat, idDocument);
                    controller.CreerExemplaire(exemplaire);
                }
            }
        }
        ...
        {
            string idCmdDvd = txbNumeroCommandeDvdRechercher.Text;
            lesCommandes = controller.GetCommandes(idCmdDvd);
            RemplirCmdDvdListe(lesCommandes);
            AccesReceptionDvdGroupBox(true);
        }
        ...
        /// </summary>
        /// <param name="acces">true ou false</param>
        private void AccesReceptionDvdGroupBox(bool acces)
        {
            grpCommandeDvd.Enabled = acces;
            txbNumCommandeDvd.Text = "";
            txbCommandeDvdMontant.Text = "";
            txbCommandeDvdNbExemplaire.Text = "";
            dtpCommandeDvdDate.Value = DateTime.Now;
        }

        /// <summary>
        /// Affichage des informations du dvd sélectionné
        /// </summary>
        /// <param name="dvd">la revue</param>
        private void AfficheReceptionDvdsInfos(Dvd dvd)
        {
        ...
        {
            if (!txbNumeroCommandeDvdRechercher.Text.Equals(""))
            {
                Dvd dvd = lesDvd.Find(x => x.Id.Equals(txbNumeroCommandeDvdRechercher.Text));
                if (dvd != null)
                {
                    AfficheReceptionDvdsInfos(dvd);
                    AccesReceptionDvdGroupBox(true);
                }
                else
                {
                    MessageBox.Show("numéro introuvable");
                    txbNumeroCommandeDvdRechercher.Focus();
                }
            }
        }
        ...
        private void RemplirCmdDvdListe(List<CommandeDocument> commandes)
        {
            ...
            {
                bdgCommandesListe.DataSource = commandes;
                dgvDvdCommandesListe.DataSource = bdgCommandesListe;
                dgvDvdCommandesListe.Columns["id"].Visible = false;
                dgvDvdCommandesListe.Columns["idLivreDvd"].Visible = false;
                dgvDvdCommandesListe.Columns["idSuivi"].Visible = false;
                dgvDvdCommandesListe.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells;
                dgvDvdCommandesListe.Columns["dateCommande"].DisplayIndex = 0;
                dgvDvdCommandesListe.Columns["montant"].DisplayIndex = 1;
            }
            else
            {
                bdgCommandesListe.DataSource = null;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void dgvDvdCommandesListe_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            string titreColonne = dgvDvdCommandesListe.Columns[e.ColumnIndex].HeaderText;
            List<CommandeDocument> sortedList = new List<CommandeDocument>();
            switch (titreColonne)
            {
                case "DateCommande":
        ...
        {
            if (!txbNumCommandeDvd.Text.Equals("") || !txbNumeroCommandeDvdRechercher.Text.Equals(""))
            {
                try
                {
                    string id = txbNumCommandeDvd.Text;
                    DateTime dateCommande = dtpCommandeDvdDate.Value;
                    string montant = txbCommandeDvdMontant.Text;
                    int nbExemplaire = int.Parse(txbCommandeDvdNbExemplaire.Text);
                    string idLivreDvd = txbNumeroCommandeDvdRechercher.Text;
                    CommandeDocument commandedocument = new CommandeDocument(id, dateCommande, montant, nbExemplaire, idLivreDvd, "00001", ENCOURS);
                    if (controller.CreerCommandeDocument(commandedocument))
                    {
                        AfficheReceptionCommandesDvd();
                    }
                ...
                catch
                {
                    MessageBox.Show("le nombre d'exemplaire doit être numérique", "Information");
                    txbCommandeDvdNbExemplaire.Text = "";
                    txbCommandeDvdNbExemplaire.Focus();
                }
            }
            else
            {
                MessageBox.Show("numéro du dvd commandé obligatoire", "Information");
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void dgvDvdCommandesListe_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                CommandeDocument commandeDocument = (CommandeDocument)bdgCommandesListe.List[bdgCommandesListe.Position];
                grpModifierSuiviCommandeDvd.Visible = true;
        ...
        private void AfficherSuiviCommandeDvd(CommandeDocument commandeDocument)
        {
            ...
            {
                VideLstSuiviCommandeDvd();
                lstSuiviCommandeDvd.Items.Add(LIVREE);
                lstSuiviCommandeDvd.Items.Add(RELANCEE);
            }
            else if (commandeDocument.IdSuivi == "00002")
            {
                VideLstSuiviCommandeDvd();
                lstSuiviCommandeDvd.Items.Add(REGLEE);
            }
            else if (commandeDocument.IdSuivi == "00003")
            {
                VideLstSuiviCommandeDvd();
                lstSuiviCommandeDvd.Items.Add("Votre commande est réglée");
            }
            else if (commandeDocument.IdSuivi == "00004")
            {
                VideLstSuiviCommandeDvd();
        ...
        private void ModifierSuiviCommandeDvd(CommandeDocument commandedocument)
        {
            ...
            else if (lstSuiviCommandeDvd.SelectedItem.ToString() == LIVREE)
            {
                commandedocument.IdSuivi = "00002";
                commandedocument.Suivi = LIVREE;
            }
            else if (lstSuiviCommandeDvd.SelectedItem.ToString() == REGLEE)
            {
                commandedocument.IdSuivi = "00003";
                commandedocument.Suivi = REGLEE;
            }
            else if (lstSuiviCommandeDvd.SelectedItem.ToString() == RELANCEE)
            {
                commandedocument.IdSuivi = "00004";
                commandedocument.Suivi = RELANCEE;
            }
        ...
        {
            CommandeDocument commandeDocument = (CommandeDocument)bdgCommandesListe.List[bdgCommandesListe.Position];
            ModifierSuiviCommandeDvd(commandeDocument);
            AfficherSuiviCommandeDvd(commandeDocument);
            AfficheReceptionCommandesDvd();
        }
        ...
        private readonly BindingSource bdgAbonnementListe = new BindingSource();
        ...
        private void AccesCommandeRevueGroupBox(bool acces)
        {
            ...
            txbCommandeNumeroRevue.Text = "";
            txbCommandeRevueMontant.Text = "";
            dtpCommandeRevueDateCommande.Value = DateTime.Now;
            dtpCommandeRevueFinAbo.Value = DateTime.Now;
        }

        /// <summary>
        /// Affichage des informations de la sélectionné
        /// </summary>
        /// <param name="revue">la revue</param>
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnRechercheRevueNumero_Click(object sender, EventArgs e)
        {
            if (!txbCommandeRevueNumeroRecherche.Text.Equals(""))
            {
                Revue revue = lesRevues.Find(x => x.Id.Equals(txbCommandeRevueNumeroRecherche.Text));
                if (revue != null)
                {
                ...
                {
                    MessageBox.Show("numéro introuvable");
                    txbCommandeRevueNumeroRecherche.Focus();
                }
            }
        }
        ...
        private void RemplirCommandeRevueListe(List<Abonnement> abonnements)
        {
            if (abonnements != null)
            {
                bdgAbonnementListe.DataSource = abonnements;
                dgvCommandesRevueListe.DataSource = bdgAbonnementListe;
                dgvCommandesRevueListe.Columns["id"].Visible = false;
                dgvCommandesRevueListe.Columns["idRevue"].Visible = false;
                dgvCommandesRevueListe.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells;
                dgvCommandesRevueListe.Columns["dateCommande"].DisplayIndex = 0;
                dgvCommandesRevueListe.Columns["montant"].DisplayIndex = 1;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void dgvCommandesRevueListe_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            string titreColonne = dgvCommandesRevueListe.Columns[e.ColumnIndex].HeaderText;
        ...
        private void txbCommandeRevueNumeroRecherche_TextChanged(object sender, EventArgs e)
        {
            ...
            pcbCommandeRevueImage.Image = null;
            RemplirCommandeRevueListe(null);
            AccesCommandeRevueGroupBox(false);
            btnSupprimerCommandeRevue.Enabled = false;
        }
        /// <summary>
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnCommandeRevueValider_Click(object sender, EventArgs e)
        {
            if (!txbCommandeNumeroRevue.Text.Equals("") || !txbCommandeRevueNumeroRecherche.Text.Equals(""))
            {
                try
                {
                    string id = txbCommandeNumeroRevue.Text;
                    DateTime dateCommande = dtpCommandeRevueDateCommande.Value;
                    string montant = txbCommandeRevueMontant.Text;
                    DateTime dateFinAbonnement = dtpCommandeRevueFinAbo.Value;
                    string idRevue = txbCommandeRevueNumeroRecherche.Text;
                    Abonnement abonnement = new Abonnement(id, dateCommande, montant, dateFinAbonnement, idRevue, null);
                ...
                {
                    MessageBox.Show("la demande n'a pas pu être effectuée", "Erreur");
                }
            }
            else
            {
                MessageBox.Show("numéro de la revue commandée obligatoire", "Information");
            }
        }
        ...
        private void btnSupprimerCommandeRevue_Click(object sender, EventArgs e)
        {
            ...
            {
                Abonnement abonnement = (Abonnement)bdgAbonnementListe.List[bdgAbonnementListe.Position];
                List<Exemplaire> desExemplaires = controller.GetExemplairesRevue(abonnement.IdRevue);
                bool supprimer = false;
                foreach(Exemplaire exemplaire in desExemplaires)
                {
                ...
                if (supprimer)
                {
                    if (MessageBox.Show("Voulez-vous vraiment supprimer la commande " + abonnement.Id + " ?", "Confirmation de suppression", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        controller.SuppAbonnement(abonnement);
                        AfficheInfoCommandesRevue();
                    }
                }
                else
                {
                    MessageBox.Show("La commande sélectionnée est rattaché à un exemplaire, elle ne peut pas être supprimée.", "Information");
                }
            }
        }
